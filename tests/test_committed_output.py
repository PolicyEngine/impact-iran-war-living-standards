...
def test_every_data_binding_in_the_methodology_tab_resolves(results):
    """The methodology tab reads figures from the results file by path. A
    renamed key, a mistyped path or a changed schema would render a blank or
    a malformed figure rather than failing the build (#53 review A2).

    The dashboard has no render tests, so this parses the component for
    `data.…` paths and the aliases assigned from them, and asserts each one
    resolves. It cannot catch a path that resolves to the WRONG value, only
    one that resolves to nothing — which is the failure mode a schema change
    produces.
    """
    import re
    from pathlib import Path
    ...
    # JS methods called on a resolved value are not part of the data path.
    JS_METHODS = {
        "toLocaleString",
        "toFixed",
        "map",
    ...
    def resolve(path_parts):
        node = results
        for part in path_parts:
            if part in JS_METHODS:
                return node  # reached the value; the rest is formatting
    ...
    for expression in sorted(set(re.findall(r"\bdata(?:\??\.\w+){2,}", component))):
        parts = expression.replace("?.", ".").split(".")[1:]
        if resolve(parts) is None:
            unresolved.append(expression)

    # Aliases assigned from `data`, so their uses resolve too. The negative
    # lookbehind stops `baseline` matching inside `data.baseline`.
    aliases = dict(re.findall(r"const (\w+) = (data(?:\??\.\w+)+);", component))
    for alias, target in aliases.items():
        base = target.replace("?.", ".").split(".")[1:]
        pattern = rf"(?<![.\w]){alias}(?:\??\.\w+)+"
        for use in sorted(set(re.findall(pattern, component))):
            tail = use[len(alias):].replace("?.", ".").split(".")[1:]
            if resolve(base + tail) is None:
                unresolved.append(use)

    assert not unresolved, (
        "these methodology-tab bindings do not resolve against the committed "
        f"results file: {sorted(set(unresolved))}"
    )
