  ...
  const comparisonRows = useMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        note: `Our figures are additions to CPI, so they compare with the shock-addition estimates above (OBR, NIESR) rather than with total-CPI levels. Our low (+${low.params.cpi_increase_pp}pp) matches the OBR view of the shock as it stands. Our central (+${central.params.cpi_increase_pp}pp) now sits just above the top of NIESR's +1pp to +3pp range, and our high (+${severe.params.cpi_increase_pp}pp) well above it, because each adder is set to at least the first-round effect of that scenario's own energy, fuel and food assumptions on ONS basket weights; for central that floor is 3.06pp, above the whole NIESR range. A scenario cannot assume less inflation than its own prices mechanically imply. It is a judgemental tail-risk assumption rather than a published UK figure, extrapolated from the Oxford Economics escalation case, which reports a 5.8% peak in world CPI with no stated equation linking that to a UK addition. Other severe published scenarios exist on a total-CPI basis and are not directly comparable with an addition.`,
      },
    ];
  }, [data]);
